...
public class MovieDao extends AbstractMFlixDao {
    ...
    public final static String EXADECIMAL = "^[A-Fa-f0-9]+$";
    ...
    public List<Document> getMoviesByCountry(String... country) {
        ...
        List<Document> movies = new ArrayList<>();
        moviesCollection.find(queryFilter).projection(fields(include("title"))).into(movies);
    ...
    public List<Document> getMoviesByCast(String sortKey, int limit, int skip, String... cast) {
        ...
        //sTicket: Subfield Text Search - implement the expected cast
        // filter and sort
        List<Document> movies = new ArrayList<>();
        moviesCollection
                .find(castFilter)
    ...
    public List<Document> getMoviesByGenre(String sortKey, int limit, int skip, String... genres) {
        ...
        moviesCollection.find(castFilter).limit(limit).skip(skip).sort(sort).into(movies);//.iterator()
       //.forEachRemaining(movies::add);
        return movies;
    }
